...
import {
    ...
    Repay as RepayEvent, SetMonetaryPolicy
...
export function handleSetMonetaryPolicy(event: SetMonetaryPolicy): void {
    ...
    if (!market) {
        log.error("Error: monetary policy {} from non existent market {} at {}", [event.params.monetary_policy.toHexString(), event.address.toHexString(), event.transaction.hash.toHexString()])
        return
    }
    let policy = new MonetaryPolicy(
        event.params.monetary_policy)
    policy.save()
