...
export function getOrCreatePolicy(policy_address: Address): MonetaryPolicy {
    ...
    if (!policy) {
        let policyContract = MonetaryPolicyAbi.bind(policy_address)
...
function getPegKeepers(policyContract: MonetaryPolicyAbi, keepers: Bytes[]): Bytes[] {
    let index = 0;
    while (true) {
        let pegKeeper = policyContract.try_peg_keepers(BigInt.fromI32(index))
        if (pegKeeper.reverted || pegKeeper.value == Address.zero()) {
            break
        }
        log.info("Found peg keeper {} for policy {}", [pegKeeper.value.toHexString(), policyContract._address.toHexString()])
        PegKeeperTemplate.create(pegKeeper.value)
        let keeper = new PegKeeper(pegKeeper.value)
        keeper.active = true
