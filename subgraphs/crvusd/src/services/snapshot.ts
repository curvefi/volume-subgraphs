...
import {
    Address, BigDecimal, BigInt, Bytes,
    ethereum,
    log,
...
function aggregateCalls(target: Address, inputValueTypes: string[][]): BigInt[] | null {
    ...
    for (let i = 0; i < inputValueTypes.length; i++) {
        params.push(changetype<ethereum.Tuple>([ethereum.Value.fromAddress(target), ethereum.Value.fromBytes(Bytes.fromHexString(inputValueTypes[i][0]))]))
    }
...
function getKeepersDebt(policyAddress: Address): BigInt {
    ...
    for (let i=0; i<policy.keepers.length;i++) {
        ...
        if (!keeper) {
            log.error("Unable to load keeper entity {} for policy {}", [policy.keepers[i].toHexString(), policyAddress.toHexString()])
            continue
        }
...
function makeBands(snapshot: Snapshot, colPrecision: string): void {
    ...
    let minBand = snapshot.minBand.toI32()
    let maxBand = snapshot.maxBand.toI32()
    ...
    let resultIndex = 0;
    while (currentBand <= maxBand) {
        let band = new Band(snapshot.id + currentBand.toString())
